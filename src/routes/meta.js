...
const fetchMetadata = async (almostExpiredSpan) => {
  ...
  data.expiredStock = (
    await StockEntry.find({
      bestBefore: { $lte: now },
    })
  ).map((e) => objs.makeStockObject(e));

  // list of almost expired stock entries
  data.amostExpiredStock = (
    await StockEntry.find({
      bestBefore: { $lte: addDays(now, almostExpiredSpan) },
    })
  ).map((e) => objs.makeStockObject(e));
  ...
  allItems.forEach(async (i) => {
    const totalStock = await StockEntry.countDocuments({
      $and: [{ item: i._id }, { consumed: false }],
    });
    if (totalStock < i.targetStock) {
      data.belowStockItems.push(await objs.makeItemObject(i));
    }
  });
  ...
  allGroups.forEach(async (g) => {
    ...
    let groupStock = 0;
    itemsInGroup.forEach(async (i) => {
      groupStock += await StockEntry.countDocuments({
        $and: [{ item: i._id }, { consumed: false }],
      });
    });
  ...
  data.latestActions = (
    await HistoryEntry.find().sort({ date: -1 }).limit(10)
  ).map((h) => objs.makeHistoryObject(h));

  // 5 most recently changed / added items
  const itemChanges = await Item.find().sort({ updatedAt: -1 }).limit(5);
  data.recentItemChanges = await Promise.all(
    itemChanges.map(async (i) => await objs.makeItemObject(i))
  );

  // 5 mos recently changed / added groups
  data.recentGroupChanges = (
    await Group.find().sort({ updatedAt: -1 }).limit(5)
  ).map((g) => objs.makeGroupObject(g));

  // 5 most recently changed / added categories
  data.recentCategoryChanges = (
    await Category.find().sort({ updatedAt: -1 }).limit(5)
  ).map((c) => objs.makeCategoryObject(c));

  // 5 most recently changed / added locations
  data.recentLocationChanged = (
    await StorageLocation.find().sort({ updatedAt: -1 }).limit(5)
  ).map((l) => objs.makeLocationObject(l));
...
module.exports = (server) => {
  server.get("/api/meta", async (req, res, next) => {
    ...
    try {
      const data = await fetchMetadata(almostExpiredSpan);
      res.send(data);
      next();
    } catch (error) {
